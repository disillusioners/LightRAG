...
def _build_client():
    ...
    try:
        ...
        args = parse_args()
        # In workspace-isolation v2, LightRAG is constructed inside
        # WorkspaceManager._create_rag_instance, so the test must patch the
        # binding used there (``lightrag.api.workspace_manager.LightRAG``)
        # rather than ``lightrag.api.lightrag_server.LightRAG`` (which is
        # no longer re-exported at the server module level).
        with patch("lightrag.api.workspace_manager.LightRAG") as mock_rag:
...
class TestQueryStreamResponseContentType:
    ...
    def test_stream_response_has_ndjson_content_type(self):
        ...
        try:
            ...
            with patch(
                "lightrag.api.workspace_manager.LightRAG", return_value=mock_rag
            ):
                app = create_app(args)
...
class TestQueryStreamProtocolOrder:
    ...
    def _build_client_with_mock(query_error: Exception | None = None):
        ...
        with patch("lightrag.api.workspace_manager.LightRAG", return_value=mock_rag):
            app = create_app(args)
    ...
    async def test_disconnect_awaits_background_query_cancellation(self):
        ...
        router = create_query_routes(FakeWorkspaceManager(HangingRag()))
        endpoint = next(
            route.endpoint for route in router.routes if route.path == "/query/stream"
        )
        # In workspace-isolation v2, ``query_text_stream`` requires the
        # ``http_request`` as the first positional arg so it can derive
        # the workspace name via ``get_workspace_from_request``. The fake
        # workspace manager ignores the workspace value, so a minimal
        # MagicMock stands in for the FastAPI ``Request`` — it only needs
        # ``.headers.get("LIGHTRAG-WORKSPACE", "")`` to return a string
        # so the sanitizer regex receives a string and not a Mock object.
