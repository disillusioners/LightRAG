...
def create_app(args):
    ...
    # Workspace isolation requires every workspace to get its own rag instance,
    # so the factory wraps the constructor and also wires per-instance setup
    # (role-LLM builder registration, role-provider logging).
    def create_lightrag(workspace: str):
        ...
        # Register role-LLM builder for runtime binding/model updates.
        # Must be on every rag instance since workspace isolation creates per-workspace
        # instances — a single global registration would miss non-default workspaces.
        ...
        # Log sanitized provider options for role LLMs on this workspace's instance.
    ...
    # Note: _log_role_provider_options and register_role_llm_builder are wired
    # inside the create_lightrag factory above so every per-workspace rag
    # instance gets them — a single global registration would miss non-default
    # workspaces under workspace isolation.
    ...
    async def get_status(request: Request):
        ...
        try:
            ...
            # (workspace isolation outer layer — refcount acquired here, released in finally)
            try:
            ...
            try:
                ...
                # Extract pipeline status fields (upstream's richer health surface)
                ...
                return {
                    ...
                    "configuration": {
                        ...
                        "enable_llm_cache": args.enable_llm_cache,
                        "vlm_process_enable": args.vlm_process_enable,
                        # workspace: the per-request workspace (resolved from header or default).
                        # This is the workspace the request was actually served from, not just
                        # the server's default — useful when callers pass LIGHTRAG-WORKSPACE.
                        "workspace": workspace,
