...
def create_query_routes(workspace_mgr, api_key: Optional[str] = None, top_k: int = 60):
    ...
    async def query_text_stream(request: QueryRequest, http_request: Request):
        ...
        **Error Handling:**
            ...
            if include_progress:
                ...
                async def merged_generator():
                    try:
                        ...
                        # Workspace release: drop the per-workspace LightRAG ref
                        # only after both progress + content streams finish so a
                        # concurrent acquire-miss does not tombstone the rag while
                        # the body is still in flight. Mirrors the inner-finally
                        # release in the default-path _generate() below.
                        await _release_once()
        ...
        # ``merged_generator()`` (progress path) and ``_generate()`` (default path)
        # ``finally`` is the sole release point for the success path; the
        # ``except Exception`` / ``except BaseException`` arms cover the error/cancel
